...
const { sendEmail_OrderCreated, sendEmail_OrderPurchasedAndBill } = require("../utils/mail_service");
  ...
  Project.findOne({ urlTitle: urlTitle })
    .then((project) => {
      if (project) res.json(project);
        ...
        return next(
          new HttpError(
            "Nepodařilo se načíst projekt (nejspíše špatný url)",
            500
          )
        );
    })
...
exports.getNewsItem = async (req, res, next) => {
  ...
  News.findById(newsId)
    ...
    .catch((err) => next(new HttpError("Nepodařilo se načíst aktualitu", 500)));
};
...
exports.getDonations = async (req, res, next) => {
  let donations = [];
  try {
    donations = await Donation.find({ isPurchased: true }).populate(
      "donatableId"
    );
  } catch (err) {
    console.log(err);
...
exports.postCreateOrder = async (req, res, next) => {
  const totalAmount = req.body.donations.reduce(
  ...
  // Transaction for creating donations and order only together!
  const sessionDB_don_order = await mongoose.startSession();
  ...
  try {
    ...
    for (const donation of req.body.donations) {
      ...
      if (!newDonation) {
        await sessionDB_don_order.abortTransaction();
    ...
    await sessionDB_don_order.commitTransaction();
  } catch (error) {
    await sessionDB_don_order.abortTransaction();
    console.log(error);
    return next(new HttpError("Nepovedlo se vytvořit objednávku", 500));
  } finally {
  ...
  let contact = {
    name: req.body.buyingAsCompany
      ? req.body.companyInfo.title
      : req.body.contact.name + " " + req.body.contact.surname,
    street_num: req.body.certificateInfo.street_num,
    city: req.body.certificateInfo.city,
  ...
  // Zde bude vytvoření a zaslání certifikátu, pokud je vyžadován !!!
...
exports.getOrderByIdAndUUID = async (req, res, next) => {
  ...
  if (!order)
    return next(
      new HttpError(
        "Nepodařilo se najít objednávku. Nejspíše nesprávna URL adresa.",
        500
      )
    );
